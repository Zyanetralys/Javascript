...
<body>
  ...
  <div class="container">
    ...
    <div class="bpm" id="bpmDisplay">152 BPM</div>
    ...
    <div class="metadata">
      <div class="metadata-row">
        ...
        <span class="metadata-value">152 bpm</span>
      </div>
      <div class="metadata-row">
        <span class="metadata-label">key</span>
        <span class="metadata-value">f# minor</span>
      </div>
      <div class="metadata-row">
      ...
      <div class="metadata-row">
        ...
        <span class="metadata-value">zona hostil // sector 7</span>
      </div>
    </div>
  ...
  <script>
    const CONFIG = {
      ...
      BEAT: 60 / 152,
      SECTIONS: {
        ALERT: { beats: 12, intensity: 0.25, label: "alerta de movimiento" },
        CHASE: { beats: 20, intensity: 0.6, label: "persecución acelerada" },
        ENGAGE: { beats: 24, intensity: 0.85, label: "contacto hostil" },
        EVAC: { beats: 16, intensity: 0.95, label: "huida táctica" }
      }
    };
    ...
    function hostileKick(time, intensity = 1) {
      ...
      gain.gain.linearRampToValueAtTime(0.6 * intensity, time + 0.004);
    ...
    function snareCrack(time, intensity = 0.8) {
      ...
      tone.type = 'triangle'; tone.frequency.value = 280;
      toneGain.gain.setValueAtTime(0.3 * intensity, time);
      toneGain.gain.exponentialRampToValueAtTime(0.01, time + 0.06);
      
      filter.type = 'bandpass'; filter.frequency.value = 1800; filter.Q.value = 1.5;
      noiseGain.gain.setValueAtTime(0, time);
      noiseGain.gain.linearRampToValueAtTime(0.5 * intensity, time + 0.002);
    ...
    function rushHat(time, intensity = 0.5) {
      const noise = audioCtx.createBufferSource();
      ...
      const gain = audioCtx.createGain();
      const filter = audioCtx.createBiquadFilter();
      filter.type = 'highpass'; filter.frequency.value = 9500;
      
      gain.gain.setValueAtTime(0, time);
      gain.gain.linearRampToValueAtTime(0.32 * intensity, time + 0.001);
    ...
    function hostileBass(time, note, duration, intensity = 0.6) {
      ...
      osc1.frequency.value = note; osc2.frequency.value = note * 1.003;
      ...
      filter.type = 'lowpass'; filter.frequency.value = 450; filter.Q.value = 0.8;
      gain.gain.setValueAtTime(0, time);
      gain.gain.linearRampToValueAtTime(0.28 * intensity, time + 0.05);
    ...
    function alarmStab(time, intensity = 0.5) {
      const osc = audioCtx.createOscillator();
      const gain = audioCtx.createGain();
      const filter = audioCtx.createBiquadFilter();
      ...
      osc.frequency.linearRampToValueAtTime(480, time + 0.15);
      
      filter.type = 'bandpass'; filter.frequency.value = 1200; filter.Q.value = 4;
      gain.gain.setValueAtTime(0, time);
      gain.gain.linearRampToValueAtTime(0.18 * intensity, time + 0.01);
    ...
    function tensionSweep(time, duration, intensity = 0.4) {
      const noise = audioCtx.createBufferSource();
      ...
      filter.frequency.exponentialRampToValueAtTime(4000, time + duration * 0.8);
      filter.Q.value = 2;
      
      gain.gain.setValueAtTime(0, time);
      gain.gain.linearRampToValueAtTime(0.25 * intensity, time + duration * 0.6);
      gain.gain.exponentialRampToValueAtTime(0.01, time + duration);
    ...
    function scheduleSection(start, key) {
      ...
      const bassNotes = [46.25, 43.65, 41.20, 38.89, 36.71]; // F#m pentatonic descent
      ...
      for (let b = 0; b < section.beats; b++) {
        ...
        // Percusión agresiva y densa
        hostileKick(t, intensity * (bar === 0 ? 1.1 : 0.9));
        if (sixteenth === 4 || sixteenth === 12) hostileKick(t + CONFIG.BEAT * 0.5, intensity * 0.7);
        
        if (bar === 2) snareCrack(t, intensity);
        if (bar === 1 || bar === 3) snareCrack(t + CONFIG.BEAT * 0.25, intensity * 0.6);
        
        // Hi-hats en 16avos constantes para urgencia
        rushHat(t, intensity * 0.5);
        rushHat(t + CONFIG.BEAT * 0.25, intensity * 0.4);
        rushHat(t + CONFIG.BEAT * 0.5, intensity * 0.45);
        rushHat(t + CONFIG.BEAT * 0.75, intensity * 0.35);
        
        // Bajo rápido en corcheas/sincopas
        if (b % 2 === 0) hostileBass(t, bassNotes[(b/2) % 5], CONFIG.BEAT * 1.8, intensity * 0.65);
        
        // Alarmas y sweeps en picos de tensión
        if (b % 8 === 0) alarmStab(t + CONFIG.BEAT * 0.1, intensity * 0.6);
        if (key === 'EVAC' && b % 4 === 0) tensionSweep(t, CONFIG.BEAT * 2, intensity * 0.7);
        
        // Fills de caos cada 8 compases
        if (b % 8 === 6) {
          for (let i = 0; i < 4; i++) {
            const ft = t + CONFIG.BEAT * 0.5 + i * (CONFIG.BEAT * 0.25);
            snareCrack(ft, intensity * 0.5);
            rushHat(ft + CONFIG.BEAT * 0.125, intensity * 0.4);
          }
        }
      }
    }
    ...
    function animate() {
      ...
      for (let i = 0; i < bars.length; i++) {
        ...
        const h = Math.max(2, (data[idx] / 255) * 75);
