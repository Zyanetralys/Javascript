...
<body>
  ...
  <div class="container">
    ...
    <div class="bpm" id="bpmDisplay">108 BPM</div>
    ...
    <div class="metadata">
      <div class="metadata-row">
        ...
        <span class="metadata-value">108 bpm</span>
      </div>
      <div class="metadata-row">
      ...
      <div class="metadata-row">
        ...
        <span class="metadata-value">madrid</span>
      </div>
    </div>
  ...
  <script>
    const CONFIG = {
      ...
      BEAT: 60 / 108,
      SECTIONS: {
        DEPLOY: { beats: 16, intensity: 0.25, label: 'despliegue' },
        MARCH: { beats: 24, intensity: 0.55, label: 'formación' },
        ENGAGE: { beats: 32, intensity: 0.85, label: 'contacto' },
        HOLD: { beats: 16, intensity: 0.35, label: 'consolidación' }
      }
    };
    ...
    function marchKick(time, intensity = 1) {
      ...
      gain.gain.linearRampToValueAtTime(0.65 * intensity, time + 0.008);
    ...
    function tacticalSnare(time, intensity = 0.8) {
      ...
      gain.gain.setValueAtTime(0, time);
      gain.gain.linearRampToValueAtTime(0.45 * intensity, time + 0.003);
    ...
    function rimClick(time, intensity = 0.5) {
      const noise = audioCtx.createBufferSource();
      ...
      gain.gain.setValueAtTime(0, time);
      gain.gain.linearRampToValueAtTime(0.25 * intensity, time + 0.001);
    ...
    function combatDrone(time, duration, intensity = 0.4) {
      ...
      filter.frequency.linearRampToValueAtTime(900, time + duration * 0.7);
      filter.Q.value = 0.5;
      
      gain.gain.setValueAtTime(0, time);
      gain.gain.linearRampToValueAtTime(0.18 * intensity, time + 0.6);
    ...
    function scheduleSection(start, key) {
      ...
      for (let b = 0; b < section.beats; b++) {
        ...
        // Patrón militar 4/4: Kick en 1 y 3, Snare en 2 y 4
        if (bar === 0 || bar === 2) marchKick(t, intensity);
        if (bar === 1 || bar === 3) tacticalSnare(t, intensity);
        
        // Rims tácticos en contratiempos (cadencia de marcha)
        rimClick(t + CONFIG.BEAT * 0.5, intensity * 0.6);
        if (bar === 3) rimClick(t + CONFIG.BEAT * 0.75, intensity * 0.4);
        
        // Toms de relleno cada 8 compases
        if (b % 8 === 4) {
          marchKick(t + CONFIG.BEAT * 0.25, intensity * 0.5);
          tacticalSnare(t + CONFIG.BEAT * 0.75, intensity * 0.6);
        }
      }
      
      // Drone atmosférico de tensión/compañerismo
      if (key !== 'ENGAGE') {
        combatDrone(start, section.beats * CONFIG.BEAT, intensity * 0.45);
      }
    }
    ...
    function animate() {
      ...
      for (let i = 0; i < bars.length; i++) {
        ...
        const h = Math.max(2, (data[idx] / 255) * 65);
